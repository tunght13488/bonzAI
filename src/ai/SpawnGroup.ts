...
export class SpawnGroup {
                ...
                if (_.isString(outcome)) {
                    ...
                    if (Game.time % 10 === 0) {
                        console.log("SPAWN:",
                            ...
                            name,
                            "cost:",
                            SpawnGroup.calculateBodyCost(build),
                            "current:",
                            this.currentSpawnEnergy,
                            "max",
                            this.maxSpawnEnergy);
                    }
                }
    ...
    public canCreateCreep(body: string[]): boolean {
        const cost = SpawnGroup.calculateBodyCost(body);
    ...
    // for example, proportion of .5 would return the max units per cost if only want to use half of your spawning
    ...
    private getMaxSpawnEnergy() {
        const contollerLevel = this.room.controller.level;
        const extensionCount = this.extensions.length;
    ...
    private manageSpawnLog() {
        ...
        const log = this.memory.log;
        const average = log.availability / 100;
        log.availability = 0;
        /*
        ...
        const longAverage = _.sum(log.history) / 5;
