...
import {ROOMTYPE_SOURCEKEEPER} from "../WorldMap";
...
export class Agent {
    ...
    public idleOffRoad(anchor: { pos: RoomPosition } = this.mission.flag, maintainDistance = false): number {
        const offRoad = this.pos.lookForStructure(STRUCTURE_ROAD) === undefined;
        if (offRoad) return OK;

        let positions = _.sortBy(this.pos.openAdjacentSpots(), (p: RoomPosition) => p.getRangeTo(anchor));
        if (maintainDistance) {
            const currentRange = this.pos.getRangeTo(anchor);
        ...
        for (const position of positions) {
            ...
            const terrain = position.lookFor(LOOK_TERRAIN)[0] as string;
            if (terrain === "swamp") {
    ...
    public stealNearby(stealSource: string): number {
        if (stealSource === "creep") {
            const creep = _(this.pos.findInRange<Creep>(FIND_MY_CREEPS, 1))
            ...
            const structure = _(this.pos.findInRange<Structure>(this.creep.room.findStructures<Structure>(stealSource), 1))
    ...
    public idleNear(place: { pos: RoomPosition }, acceptableRange = 1, cachePos = false, allowSwamp = true): number {
        ...
        if (range <= acceptableRange + 1) {
            ...
            const direction = this.creep.pos.getDirectionTo(place);
            for (let i = -2; i <= 2; i++) {
                let relDirection = direction + i;
                relDirection = helper.clampDirection(relDirection);
                const position = this.creep.pos.getPositionAtDirection(relDirection);
                if (!position.inRangeTo(place, acceptableRange)) continue;
                if (position.lookForStructure(STRUCTURE_ROAD)) continue;
                if (!position.isPassable()) continue;
        ...
        if (range <= 1) {
            const position = this.findIdlePosition(place, acceptableRange);
            if (!position) { return; }
    ...
    private cacheIdlePosition(place: { pos: RoomPosition }, acceptableRange: number): RoomPosition {
        if (this.memory.idlePosition) {
            ...
            const range = position.getRangeTo(place);
            if (range === 0) {
                return position;
            }
            if (range <= acceptableRange && position.isPassable()) {
                return position;
            }
            else {
        ...
        while (radius <= acceptableRange) {
            for (let xDelta = -radius; xDelta <= radius; xDelta++) {
                for (let yDelta = -radius; yDelta <= radius; yDelta++) {
                    ...
                    const x = place.pos.x + xDelta;
                    const y = place.pos.y + yDelta;
                    const position = new RoomPosition(x, y, place.pos.roomName);
                    if (!position.isPassable()) { continue; }
                    if (position.isNearExit(0)) { continue; }
                    if (position.lookForStructure(STRUCTURE_ROAD)) { continue; }
    ...
    public seekBoost(boosts: string[], allowUnboosted: boolean) {
        ...
        for (const boost of boosts) {
            ...
            const requests = this.room.memory.boostRequests;
            if (!requests) {
            ...
            const boostedPart = _.find(this.creep.body, {boost});
            if (boostedPart) {
                this.memory[boost] = true;
            ...
            const flag = Game.flags[requests[boost].flagName];
            if (!flag) continue;

            const lab = flag.pos.lookForStructure(STRUCTURE_LAB) as StructureLab;
            ...
            if (lab.mineralType === boost && lab.mineralAmount >= IGOR_CAPACITY && lab.energy >= IGOR_CAPACITY) {
                ...
                if (Game.time % 10 === 0) {
                    console.log("BOOST: no boost for", this.creep.name,
    ...
    public avoidSK(destination: Flag): number {
        const costCall = (roomName: string, matrix: CostMatrix): CostMatrix | boolean => {
            if (roomName !== this.pos.roomName) return;
            const room = Game.rooms[this.pos.roomName];
            const sourceKeepers = _.filter(room.hostiles, (c: Creep) => c.owner.username === "Source Keeper");
        ...
        if (this.room.roomType === ROOMTYPE_SOURCEKEEPER) {
            ...
            const hostileCount = this.creep.room.hostiles.length;
    ...
    public fleeByPath(fleeObjects: Array<{ pos: RoomPosition }>, fleeRange: number, fleeDelay: number, confineToRoom = false): boolean {

        const closest = this.pos.findClosestByRange(fleeObjects);
        ...
        if (rangeToClosest > fleeRange) {
            ...
            const _fleeData = this.memory._flee;
        ...
        if (this.fatigue > 0) {
            if (closest instanceof Creep) {
                const moveCount = this.getActiveBodyparts(MOVE);
                const dropAmount = this.carry.energy - (moveCount * CARRY_CAPACITY);
        ...
        const fleeData = this.memory._flee as FleeData;
        fleeData.delay = fleeDelay;

        if (fleeData.nextPos) {
            const position = helper.deserializeRoomPosition(fleeData.nextPos);
        ...
        if (fleeData.path) {
            if (fleeData.path.length > 0) {
                const _nextDirection = parseInt(fleeData.path[0], 10);
                const position = this.pos.getPositionAtDirection(_nextDirection);
                if (!position.isNearExit(0) &&
                    position.findClosestByRange(fleeObjects).pos.getRangeTo(position) < rangeToClosest) {
                    fleeData.path = undefined;
                }
                else {
                    this.move(_nextDirection);
        ...
        if (!fleeData.path) {
            const avoidance = _.map(fleeObjects, obj => ({pos: obj.pos, range: 10}));

            const ret = PathFinder.search(this.pos, avoidance, {
                flee: true,
        ...
        const nextDirection = parseInt(fleeData.path[0], 10);
        fleeData.nextPos = this.pos.getPositionAtDirection(nextDirection);
        this.move(nextDirection);
        return true;
    }

    public retreat(avoidObjects?: Array<{ pos: RoomPosition }>, fleeRange = 5): number {
        if (!avoidObjects) {
            avoidObjects = this.room.fleeObjects;
        }

        const avoidance = _.map(this.pos.findInRange(avoidObjects, fleeRange + 1),
            (c: Creep) => ({pos: c.pos, range: 20}));
    ...
    public moveItOrLoseIt(position: RoomPosition, name?: string, lethal = true): number {
        ...
        const range = this.pos.getRangeTo(position);
        if (range === 0) return OK;
        if (range > 1) { return this.travelTo(position) as number; }

        // take care of creep that might be in the way
        const occupier = _.head(position.lookFor<Creep>(LOOK_CREEPS));
        if (occupier && occupier.name) {
            if (name && occupier.name.indexOf(name) >= 0) {
                if (lethal) {
                    for (const resourceType in occupier.carry) {
                        const amount = occupier.carry[resourceType];
                        if (amount > 0) {
                ...
                const _direction = occupier.pos.getDirectionTo(this);
                occupier.move(_direction);
                this.creep.say("move it");
            }
        }

        // move
        const direction = this.pos.getDirectionTo(position);
    ...
    public yieldRoad(target: { pos: RoomPosition }, allowSwamps = true): number {
        const isOffRoad = this.pos.lookForStructure(STRUCTURE_ROAD) === undefined;
        ...
        for (let i = -2; i <= 2; i++) {
            let relDirection = direction + i;
            relDirection = helper.clampDirection(relDirection);
            const position = this.pos.getPositionAtDirection(relDirection);
     ...
     * @returns {number}
     */
    public withdrawIfFull(target: Creep | StructureContainer | StructureStorage | StructureTerminal, resourceType: string): number {
        if (!this.pos.isNearTo(target)) {
            return ERR_NOT_IN_RANGE;
        }

        const norm = Agent.normalizeStore(target);
        const storageAvailable = this.carryCapacity - _.sum(this.carry);
        const targetStorageAvailable = norm.storeCapacity - _.sum(norm.store);
    ...
    public withdrawEverything(target: Creep | StructureContainer | StructureStorage | StructureTerminal): number {
        const norm = Agent.normalizeStore(target);
        for (const resourceType in norm.store) {
            const amount = norm.store[resourceType];
            if (amount > 0) {
                return this.withdraw(target, resourceType);
            }
        }
    ...
    public rememberStructure(findStructure: () => Structure, forget: (structure: Structure) => boolean,
        ...
        if (this.memory[prop]) {
            const structure = Game.getObjectById(this.memory[prop]) as Structure;
    ...
    public rememberCreep(findCreep: () => Creep, forget: (creep: Creep) => boolean): Creep {
        if (this.memory.remCreepId) {
            const creep = Game.getObjectById(this.memory.remCreepId) as Creep;
    ...
    public rememberBattery(): Creep | StructureContainer {
        if (this.memory.batteryId) {
            const battery = Game.getObjectById(this.memory.batteryId) as Creep | StructureContainer;
            ...
            const battery = this.room.getAltBattery(this.creep);
            if (battery) {
    ...
    public recycleSelf(container: StructureContainer) {
        ...
        const binTooFull = (this.ticksToLive + _.sum(container.store)) > container.storeCapacity;
        if (binTooFull) {
        ...
        const spawn = this.pos.findClosestByRange(FIND_MY_SPAWNS) as StructureSpawn;
        if (!spawn) {
            console.log("recycleBin is missing spawn in", this.room.name);
            return;
        }

        const recycleOutcome = spawn.recycleCreep(this.creep);
    ...
    public procureEnergy(nextDestination?: { pos: RoomPosition }, highPriority = false, getFromSource = false) {
        ...
        if (battery) {
            ...
            if (getFromSource) {
                const closest = this.pos.findClosestByRange<Source>(this.mission.sources);
                if (closest) {
                ...
                if (this.memory._travel && this.memory._travel.dest) {
                    const destPos = this.memory._travel.dest;
                    const dest = new RoomPosition(destPos.x, destPos.y, destPos.roomName);
    ...
    public nextPositionInPath(): RoomPosition {
        if (this.memory._travel && this.memory._travel.path && this.memory._travel.path.length > 0) {
            const position = this.pos.getPositionAtDirection(parseInt(this.memory._travel.path[0], 10));
            if (!position.isNearExit(0)) {
                return position;
            }
        }
    ...
    public getBattery(): Creep | StructureContainer | StructureTerminal | StructureStorage {
        const minEnergy = this.carryCapacity - this.carry.energy;
    ...
    public standardHealing(agents: Agent[]): boolean {
        ...
        if (hurtAgents.length > 0) {
            ...
            const mostHurt = _.head(hurtAgents);
            if (mostHurt.pos.isNearTo(this)) {
                this.heal(mostHurt);
                return true;
            }

            const nearbyAndHurt = _.filter(this.pos.findInRange(hurtAgents, 1),
    ...
    public standardRangedAttack(): Creep {
        const hostilesInRange = _(this.pos.findInRange<Creep>(FIND_HOSTILE_CREEPS, 3))
    ...
    public standardMelee(damageThreshold = 0): Creep {
        ...
        const hostilesInRange = _(this.pos.findInRange<Creep>(FIND_HOSTILE_CREEPS, 1))
            .sortBy(creep => creep.hits - creep.hitsMax)
            .value();
        if (hostilesInRange.length > 0) {
    ...
    public moveOffExit(): number {
        ...
        for (let direction = 1; direction < 8; direction++) {
            const position = this.pos.getPositionAtDirection(direction);
            if (position.isNearExit(0)) { continue; }
            if (!position.isPassable()) { continue; }
    ...
    public pushyTravelTo(destination: { pos: RoomPosition }, exclusion?: string, options: TravelToOptions = {}) {
        if (this.isStuck()) {
            ...
            if (options.returnData.nextPos) {
                const creep = options.returnData.nextPos.lookFor<Creep>(LOOK_CREEPS)[0];
