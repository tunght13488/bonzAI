...
export abstract class Operation {
    ...
    public spawnData: {
        spawnRooms: Array<{ distance: number, roomName: string }>;
     ...
     * @param name - second part of flag.name, should be unique amount all other operation names (I use city names)
    ...
    public actions() {
        ...
        for (const missionName in this.missions) {
            try {
                ...
                console.log("error caught in missionActions phase, operation:",
                    this.name,
                    "mission:",
                    missionName,
                    "in missionRoom ",
        ...
        if (Math.random() < .01) {
            for (const missionName in this.missions) {
                try {
                    ...
                    console.log("error caught in invalidateMissionCache phase, operation:",
                        this.name,
                        "mission:",
                        missionName);
                    console.log(e.stack);
    ...
    public setMax(missionName: string, max: number) {
        ...
        const oldValue = this.memory[missionName].max;
    ...
    public setBoost(missionName: string, activateBoost: boolean) {
        if (!this.memory[missionName]) return "SPAWN: no " + missionName + " mission in " + this.name;
        const oldValue = this.memory[missionName].activateBoost;
    ...
    protected findOperationWaypoints() {
        this.waypoints = [];
        for (let i = 0; i < 100; i++) {
            const flag = Game.flags[this.name + "_waypoints_" + i];
            if (flag) {
